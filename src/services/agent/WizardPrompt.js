...
2. **PLAN**: Create high-level steps first. For complex steps (large graphs, many nodes), you can optionally add \`substeps\` to break down the specific work — e.g., which nodes to add, which edges to create, which sections of the graph to build. Substeps are useful for chunking large work but not required for straightforward steps. Substeps auto-complete their parent step when all substeps are done.
