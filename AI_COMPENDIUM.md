...
| **Change the canvas: NodeCanvas, anything it renders, or the canvas hooks** | `documentation/core-system/CANVAS_ARCHITECTURE.md` (current — stores, layers, hosts, controllers, the pie machine, where new features go). For why it is shaped this way, or to pick up a deferred card: `documentation/dev-ops/nodecanvas-refactor/README.md` (historical — the closed pre-1.0 refactor). Supersedes the Oct-2025 `.refactor-*.md` files |
